...
# НОВЫЙ РАЗДЕЛ ШИФРОВАНИЯ И РАСШИФРОВАНИЯ ФАЙЛОВ
    ...
    def encrypt_file(self, input_file_path: str, output_file_path: str,
        ...
        try:
            ...
            with open(input_file_path, 'rb') as infile, open(output_file_path, 'wb') as outfile:
                # Блочное чтение файла, при этом 1 МБ за раз
                ...
                while True:
                    ...
                    # Шифрование блока
                    encrypted_block = self.encrypt_data(block)
        ...
        try:
            ...
            with open(input_file_path, 'rb') as infile, open(output_file_path, 'wb') as outfile:
                ...
                while True:
                    ...
                    # Расшифрование блока
                    decrypted_block = self.decrypt_data(block)
    ...
    def encrypt_string(self, text: str) -> str:
        ...
        Результат на выходе:
            Зашифрованная строка в hex формате
        """
        encrypted_bytes = self.encrypt_data(text.encode('utf-8'))
        return encrypted_bytes.hex()

    def decrypt_string(self, hex_string: str) -> str:
        """
        Расшифрование hex строки в текст.

        Исходный параметр:
            hex_string: hex представление зашифрованного текста
        Результат на выходе:
        ...
        try:
            ...
            decrypted_bytes = self.decrypt_data(bytes.fromhex(hex_string))
            return f"[Бинарные данные, {len(decrypted_bytes)} байт]"

    @staticmethod
    def validate_key_length(key_bytes: bytes) -> bool:
        """
        Проверка длины ключа.

        Исходный параметр:
            key_bytes: ключ в байтах
        Результат на выходе:
            True если длина корректна, False в противном случае
        """
        return MIN_KEY_LEN <= len(key_bytes) <= MAX_KEY_LEN

    @staticmethod
    def validate_iv_length(iv_bytes: bytes) -> bool:
        """
        Проверка длины IV.

        Исходный параметр:
            iv_bytes: IV в байтах
        Результат на выходе:
            True если длина корректна, False в противном случае
        """
        return len(iv_bytes) <= MAX_IV_LEN

    def reset_state(self):
        """
        Сброс состояния шифра для начала новой сессии.
        """
        self.buf_position = 0
        self.keystream_buf = None
    ...
    def get_key_info(self) -> dict:
        ...
        return {
            ...
            'key_bits': len(self.key) * 8 if hasattr(self, 'key') else 0,
            'iv_length': len(self.iv) if hasattr(self, 'iv') else 0,
            'iv_bits': len(self.iv) * 8 if hasattr(self, 'iv') else 0
        }
    ...
    def __str__(self) -> str:
        ...
        Результат на выходе:
            Информация о состоянии шифра
        """
        key_info = self.get_key_info()
        return (f"CustomSosemanuk(key={key_info['key_bits']} bits, "
                f"iv={key_info['iv_bits']} bits, "
                f"buf_position={self.buf_position})")

    def encrypt_file_simple(self, input_file_path: str, output_file_path: str) -> bool:
        """
        Простое шифрование файла без прогресса.
    ...
    def decrypt_file_simple(self, input_file_path: str, output_file_path: str) -> bool:
        """
        Простое расшифрование файла без прогресса.
